...
> ECC 子代理适配为 DSH subagent 提示词。原文件：`agents/chief-of-staff.md`

## 调用说明

- **用途**：Personal communication chief of staff that triages email, Slack, LINE, and Messenger. Classifies messages into 4 tiers (skip/info_only/meeting_info/action_required), generates draft replies, and enforces post-send follow-through via hooks. Use when managing multi-channel communication workflows.
- **工具映射**：read, grep, glob, pwsh (Windows) / bash (POSIX), edit, write
