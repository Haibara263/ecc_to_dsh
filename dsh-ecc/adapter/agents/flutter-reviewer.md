...
> ECC 子代理适配为 DSH subagent 提示词。原文件：`agents/flutter-reviewer.md`

## 调用说明

- **用途**：Flutter and Dart code reviewer. Reviews Flutter code for widget best practices, state management patterns, Dart idioms, performance pitfalls, accessibility, and clean architecture violations. Library-agnostic — works with any state management solution and tooling.
