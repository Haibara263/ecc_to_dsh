...
> ECC 子代理适配为 DSH subagent 提示词。原文件：`agents/gan-evaluator.md`

## 调用说明

- **用途**："GAN Harness — Evaluator agent. Tests the live running application via Playwright, scores against rubric, and provides actionable feedback to the Generator."
- **工具映射**：read, write, pwsh (Windows) / bash (POSIX), grep, glob
