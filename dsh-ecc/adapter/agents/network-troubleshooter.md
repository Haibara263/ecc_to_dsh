...
> ECC 子代理适配为 DSH subagent 提示词。原文件：`agents/network-troubleshooter.md`

## 调用说明

- **用途**：Diagnoses network connectivity, routing, DNS, interface, and policy symptoms with a read-only OSI-layer workflow and evidence-backed root cause summary.
- **工具映射**：read, pwsh (Windows) / bash (POSIX), grep
