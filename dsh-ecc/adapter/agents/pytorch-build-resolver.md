...
> ECC 子代理适配为 DSH subagent 提示词。原文件：`agents/pytorch-build-resolver.md`

## 调用说明

- **用途**：PyTorch runtime, CUDA, and training error resolution specialist. Fixes tensor shape mismatches, device errors, gradient issues, DataLoader problems, and mixed precision failures with minimal changes. Use when PyTorch training or inference crashes.
